...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    // Local development fallback: no database configured, but a demo session
    // should still be accepted so the app can render authenticated routes.
    if (!user && (session.appId === "local-demo-app" || session.openId === "demo-user")) {
      user = {
        id: -1,
        openId: session.openId,
        name: session.name || "Usuário demo",
        email: "demo@manus.local",
